...
void njdp2d_draw_0() {
    ...
    for (i = njdp2d_w.ix1st; i != -1; i = njdp2d_w.prim[i].next) {
        if (njdp2d_w.prim[i].type == 0) {
            ...
            for(j = 0; j < 3; j++){
                k = -j + 5;
                vertices[k].x = vertices[j].x = njdp2d_w.prim[i].v[j].x * Frame_Zoom_X + Frame_Off_X;
                vertices[k].y = vertices[j].y = njdp2d_w.prim[i].v[j].y * Frame_Zoom_Y + Frame_Off_Y;
                vertices[k].z = vertices[j].z = njdp2d_w.prim[i].v[j].z * 0xFFFF;
                vertices[k].colour = vertices[j].colour = njdp2d_w.prim[i].col;
            }
            vertices[5].x = njdp2d_w.prim[i].v[j].x * Frame_Zoom_X + Frame_Off_X;
            vertices[5].y = njdp2d_w.prim[i].v[j].y * Frame_Zoom_Y + Frame_Off_Y;
